...
$(function () {
        ...
        form.on('submit(return)', function (data) {
            layer.open({
                ...
                btn1: function (index, item) {
                    window.location.href = '../buyService/buyService.html?id=' + GetQueryString('id') + '&type=' + window.btoa('yffs') + '&level=' + window.btoa('Levelthree')
                }
            });
            return false;
        });
    ...
    // 获取参数总价
    $('._Price').html('￥' + window.atob(GetQueryString('money')));
    
    // 获取参数商品名称
    $('.name').html(decodeURI(GetQueryString('prodName')));

    $('.specifications').html(window.atob(GetQueryString('cycle')) + 'G');

    $('.zhouqi').html(window.atob(GetQueryString('specs')) + '年');
...
var ServiceDetails = function () {
    ...
    window.location.href = '../buyService/buyService.html?id=' + window.btoa(id) + '&level=' + window.btoa('Levelthree') ;
}
...
var SubmitPay = function (step) {
    ...
    request.data = JSON.stringify({
        // 属性方案
        attributeId: window.atob(GetQueryString('attributeId')),
        // 服务周期
        cycle: window.atob(GetQueryString('cycle')),
        // 纳税人识别号
        ...
        payType: parent.pay,
        // 支付金额
        money: window.atob(GetQueryString('money')) * 100,
        //商品名称
        prodName: decodeURI(GetQueryString('prodName')),
        // 相关单据ID
        relateId: window.atob(GetQueryString('id')),
        // 服务规格
        specs: window.atob(GetQueryString('specs')),
        // 订单类型
        type: '1',
        prodLogo: decodeURI(GetQueryString('imglogo')),
    })
    ...
    request.redata = function (data) {
        if (data.code === 0 ) {
            ...
            $('.mixins').html('￥' + window.atob(GetQueryString('money')));
